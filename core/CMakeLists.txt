...
void sp_path_move_to(sp_path_t* p, float x, float y) { as_path(p)->points.clear(); as_path(p)->points.push_back({x,y}); }
...
sp_font_t* sp_load_font(sp_canvas_t* c, const char* path) {
    ...
    if (!file.read((char*)font->ttf_buffer.data(), font->ttf_buffer.size())) {
        delete font;
        return nullptr;
    }
...
void sp_draw_text(sp_canvas_t* c, const char* text, float x, float y) {
    ...
    while (*ptr) {
        if (*ptr >= 32 && *ptr < 128) {
            ...
            as_canvas(c)->m_renderer->addQuad(
                {q.x0, q.y0, 0, 1}, {q.x1, q.y0, 0, 1},
                {q.x1, q.y1, 0, 1}, {q.x0, q.y1, 0, 1},
                color, tid, {q.s0, q.t0, q.s1, q.t1}
            );
        }
        ptr++;
    }
